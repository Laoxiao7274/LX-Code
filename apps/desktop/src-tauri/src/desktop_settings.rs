...
const LXCODE_DATA_DIR_NAME: &str = "lxcode";
...
impl DesktopSettingsStore {
    ...
    pub fn load(app: &AppHandle) -> Result<Self, String> {
        // LXCODE_CONFIG_DIR 优先,兼容旧 PIDECK_CONFIG_DIR(品牌迁移期)
        let dir = match std::env::var_os("LXCODE_CONFIG_DIR")
            .or_else(|| std::env::var_os("PIDECK_CONFIG_DIR"))
        {
            Some(value) => {
    ...
    pub fn ensure_default_project_workspace(&mut self) -> Result<Option<PathBuf>, String> {
        ...
        let namespace_dir = self.resolved_agent_dir().join(LXCODE_DATA_DIR_NAME);
    ...
    pub fn resolved_agent_dir(&self) -> PathBuf {
        ...
        // LXCode: 数据存 ~/.lxcode(独立,不共享 pi CLI 的 ~/.pi/agent)
        dirs::home_dir()
...
mod tests {
    ...
    fn test_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("lxcode-settings-{name}-{}", Uuid::new_v4()));
    ...
    fn initializes_and_persists_the_default_project_workspace() {
        ...
        let project_dir = agent_dir.join("lxcode").join("DefaultProject");
        let project_path = project_dir.to_string_lossy().into_owned();
        let mut store = DesktopSettingsStore::load_from_dir(&dir).unwrap();
        ...
        #[cfg(unix)]
        {
            let namespace_mode = fs::metadata(agent_dir.join("lxcode"))
                .unwrap()
                .permissions()
                .mode()
                & 0o777;
    ...
    fn preserves_every_form_of_existing_workspace_configuration() {
        ...
        for (index, settings) in cases.into_iter().enumerate() {
            ...
            assert_eq!(store.ensure_default_project_workspace().unwrap(), None);
            assert_eq!(serde_json::to_value(&store.settings).unwrap(), before);
            assert!(!agent_dir.join("lxcode").join("DefaultProject").exists());
            fs::remove_dir_all(dir).unwrap();
        }
    }
}
