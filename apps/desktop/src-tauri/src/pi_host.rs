...
impl PiHostManager {
    ...
    pub async fn begin_start(&mut self) -> Result<PendingStart, String> {
        ...
        // Tauri app_config_dir:含 desktop-settings.json(模型用途 useCases 配置在此)。
        // 传给 pi-host 进程,让内置视觉工具能读 useCases.vision 配置的视觉模型。
        let config_dir = strip_verbatim_prefix(
            self.app
                .path()
                .app_config_dir()
        ...
        eprintln!(
            "[lxcode] starting host node={} entry={} cwd={} agentDir={} cacheDir={}",
            node.display(),
            entry.display(),
            work_dir.display(),
            agent_dir.display(),
    ...
    pub async fn complete_start(&mut self, done: CompletedStart) -> Result<(), String> {
        ...
        match done.outcome {
            StartWaitOutcome::Ready(hid) => {
                eprintln!("[lxcode] host.ready received");
                if let Some(id) = hid {
                    self.host_instance_id = Some(id);
                }
    ...
    pub async fn send_line(&mut self, line: String) -> Result<(), String> {
        ...
        if payload.contains("\"method\":\"system.hello\"") {
            eprintln!("[lxcode] writing system.hello request to Host");
        }
        let result =
    ...
    async fn shutdown_with_grace(&mut self, graceful_timeout: Duration) {
        ...
        if self.stdin.is_some() {
            ...
            if self.host_instance_id.is_none() {
                eprintln!(
                    "[lxcode] shutdown without hostInstanceId — sending expectedHostInstanceId=unknown then terminate"
                );
            }
