...
import { requestDockBrowser, requestDockBrowserClose } from "../lib/dock-browser";
...
import type { HostEventEnvelope, HostEventPayloadMap } from "@lxcode/protocol";
...
function SettingsOverlay({ section }: { section: SettingsSection }) {
  ...
  // 进场:fade + 上滑,只用 transform/opacity(GPU 合成便宜),避免 scale 重内容重光栅化掉帧。
  useGSAP(() => {
    if (!overlayRef.current) return;
    gsap.fromTo(overlayRef.current, { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: 0.22, ease: "power3.out" });
  }, []);

  const requestClose = () => {
    if (useAppStore.getState().providersDirty) { setConfirmDiscard(true); return; }
    closeOverlay();
  };
  const closeOverlay = () => {
    if (!overlayRef.current) { setPage("chat"); return; }
    gsap.to(overlayRef.current, { opacity: 0, y: 16, duration: 0.18, ease: "power2.in", onComplete: () => setPage("chat") });
  };
  ...
  return (
    <div ref={overlayRef} className="absolute inset-0 z-40 bg-surface" style={{ willChange: "transform, opacity" }}>
...
export function App() {
  ...
  // pi 扩展通过 setStatus("lx:open-browser", url) 请求开内置浏览器(curator/open_browser 工具)
  const extensionStatuses = useAppStore((s) => s.extensionStatuses);
  useEffect(() => {
    const url = extensionStatuses["lx:open-browser"];
    if (url && /^https?:\/\//i.test(url)) {
      requestDockBrowser({ url });
    } else if (url === undefined) {
      // 扩展清除了 status(curator 完成/取消) → 关闭内置浏览器并收回 dock
      requestDockBrowserClose();
    }
  }, [extensionStatuses]);
