...
import type { TerminalProfileId } from "@lxcode/protocol";
...
import { subscribeDockBrowser, subscribeDockBrowserClose } from "../lib/dock-browser";
...
export function RightDock() {
  ...
  // curator/open_browser 完成后扩展清除 status → 关当前 browser tab + 收回 dock
  useEffect(
    () =>
      subscribeDockBrowserClose(() => {
        // 关掉所有 browser tab(curator/open_browser 开的)
        const tabs = browserTabsRef.current.slice();
        for (const tab of tabs) closeBrowser(tab.id);
