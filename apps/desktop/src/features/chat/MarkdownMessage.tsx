...
> {
  ...
  componentDidCatch(error: Error): void {
    ...
    console.warn("LXCode Markdown enhancement failed; using plain text", error);
  }
...
export const MarkdownMessage = memo(function MarkdownMessage({
  ...
  // 打字机匀速器:streaming 时 content 以固定速率 reveal 给 Streamdown,避免 AI 极快输出时
  // 每帧全量重解析越来越长的 markdown 导致卡顿。每帧只 +N 字,视觉匀速逐字丝滑。
  // static(历史消息)直接全量,不启动 rAF。
  const STREAMING_CHARS_PER_FRAME = 3; // ≈180 字/秒 @60fps,可调
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let cur = revealedLenRef.current;
      if (cur > target) { cur = 0; revealedLenRef.current = 0; setRevealedLen(0); }
      if (cur < target) {
        const next = Math.min(cur + STREAMING_CHARS_PER_FRAME, target);
  ...
  const displayedContent =
    mode === "streaming" ? content.slice(0, revealedLen) : content;
  const normalized = useMemo(
    () => deferIncompleteMermaid(sanitizeAgentText(displayedContent)),
    [displayedContent],
  );
