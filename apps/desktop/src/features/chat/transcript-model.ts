...
export type ParsedUserText = {
  ...
  /** [粘贴图片: <路径>] 标记提取的本地图片路径(用于渲染预览)。 */
  imagePaths: string[];
};

/** [粘贴图片: <绝对路径>] 标记正则。路径含盘符反斜杠/正斜杠,空格容忍。 */
const PASTED_IMAGE_RE = /\[粘贴图片\s*:\s*(.+?)\s*\]/g;
...
export function parseUserAttachments(raw: string): ParsedUserText {
  ...
  const text = stripAttachmentReferenceBlocks(raw)
    ...
    .replace(PASTED_IMAGE_RE, (_m, path: string) => {
      imagePaths.push(path.trim());
      return "";
    })
    .trim();
  return { text, files, documents, imagePaths };
}
