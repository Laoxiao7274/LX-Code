...
import { LxToolbar } from "./LxToolbar";
...
export function Composer({
  ...
  // textarea 自动增高:单行靠 h-10 + py-[11px] + leading-[18px] 居中(不设 inline height);
  // 多行(含换行)才按 scrollHeight 扩展,上限 120px。避免原生滚动条(右侧灰条)。
  useEffect(() => {
  ...
  async function send() {
    ...
    setDocuments([]);
    const context = activeSessionContext(host, workspace, session);
    // 粘贴/拖入的图片:存成临时文件拿路径,把路径放进消息文本。
    // AI 收到 [粘贴图片: 路径] 后调 vision_analyze 工具识别(不阻塞 input,主模型无需支持视觉)。
    // 不再作为 image attachment 发(主模型不支持视觉会被 pi 丢弃并加干扰提示)。
    const imageBlocks: string[] = [];
    for (const img of sentImages) {
      try {
        const path = await writeTempImage(img.data, img.mediaType);
        imageBlocks.push(`[粘贴图片: ${path}]`);
      } catch {
        // 存文件失败则降级为 image attachment(视觉模型可直接看)
      }
    }
    const sentImagesAsAttachment = imageBlocks.length < sentImages.length ? sentImages : [];
    const outgoingText =
      [value.trimEnd(), ...sentFiles.map((f) => buildAttachedFileBlock(f.name, f.text)), ...imageBlocks]
        .filter(Boolean)
        .join("\n\n");
    const imageParams =
      sentImagesAsAttachment.length > 0
        ? { images: sentImagesAsAttachment.map(({ mediaType, data }) => ({ mediaType, data })) }
        : {};
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`mx-auto max-w-3xl rounded-2xl border border-border/60 bg-card/95 px-2.5 py-2.5 shadow-lg backdrop-blur-sm transition-colors ${
            dragOver ? "border-accent/40" : "border-border/60"
          }`}
          onDragOver={(event) => {
            if (disabled) return;
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="mx-auto flex h-10 max-w-3xl items-center gap-1 rounded-2xl bg-card px-1.5 transition-colors">
              <button
                type="button"
                title={t("composerAttach")}
                aria-label={t("composerAttach")}
                className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-muted transition-colors hover:bg-surface-overlay hover:text-foreground disabled:opacity-40"
                disabled={
                  disabled ||
              ...
              <textarea
              ref={textareaRef}
              className="chat-composer-input block h-10 w-full resize-none border-0 bg-transparent px-1 py-[11px] text-[13px] leading-[18px] text-foreground outline-none placeholder:text-muted/60 disabled:opacity-60"
              style={{ maxHeight: 120 }}
              rows={1}
