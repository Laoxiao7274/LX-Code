...
/** 消息行进场动画:fade + 上滑,GSAP power2.out。只在挂载时跑一次。 */
const AnimatedRow = memo(function AnimatedRow({ children }: { children: ReactNode }) {
...
import { collapseStyle, collapseInnerStyle } from "./collapse-style";
...
import { stripAttachmentReferenceBlocks } from "@lxcode/protocol";
import {
  buildTranscriptRows,
...
function AssistantAvatar() {
  return <LxMark className="mt-0.5 size-7" />;
}
...
// [粘贴图片: 路径] 渲染:异步读本地图片用 data: URL 显示预览(CSP 允许 data:)。
function PastedImage({ path }: { path: string }) {
  const t = useT();
    ...
    void (async () => {
      try {
        ...
        if (file.kind === "image" && file.data) {
          setSrc(`data:${file.mediaType};base64,${file.data}`);
        } else {
          setFailed(true);
        }
      } catch {
  ...
  if (failed) {
    return (
      <div className="flex items-center gap-1.5 rounded-lg border border-border bg-surface px-2 py-1.5 text-[11px] text-muted" title={path}>
  ...
  if (!src) {
    return (
      <div className="flex h-24 w-24 items-center justify-center rounded-lg border border-border bg-surface">
        <ImageIcon size={16} className="animate-pulse text-muted" />
      </div>
    );
  }
  return (
    <img src={src} alt={t("transcriptAttachmentAlt")} title={path} className="max-h-48 max-w-full rounded-lg border border-border object-contain" />
  );
}
    ...
    return (
      <div className="group relative ml-auto w-fit max-w-[92%] sm:max-w-[78%]">
        ...
        {parsed.imagePaths.length > 0 && (
          <div className="mb-1 flex flex-wrap justify-end gap-1.5">
            {parsed.imagePaths.map((path, index) => (
              <PastedImage key={`pasted:${index}`} path={path} />
            ))}
          </div>
        )}
