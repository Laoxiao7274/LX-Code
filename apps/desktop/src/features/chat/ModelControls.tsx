import { Brain, Check, ChevronDown, Activity } from "lucide-react";
...
export function ContextUsageRing() {
  ...
  return (
    <span ref={containerRef} className="relative flex h-6 shrink-0 items-center gap-1.5 px-2 text-[11px] text-muted">
      <button
        type="button"
        className="flex h-6 items-center gap-1.5 rounded-md px-2 text-[11px] text-muted transition-colors hover:bg-surface-overlay/60"
        aria-label={title}
            ...
            <div className="h-1.5 w-14 overflow-hidden rounded-full bg-surface-overlay">
              <div
                className={`h-full rounded-full transition-all ${
                  percent > 80 ? "bg-danger" : percent > 50 ? "bg-amber-500" : "bg-accent"
                }`}
                style={{ width: `${percent}%` }}
              />
            </div>
            <span className="font-mono tabular-nums">{roundedPercent}%</span>
          </>
        )}
      </button>
      {open && (
...
export function ModelControls() {
  ...
  return (
    <div className="flex min-w-0 items-center">
      <div ref={menuRef} className="relative flex h-6 min-w-0 max-w-[280px] items-center">
        <span
        ...
        <button
          type="button"
          className="flex h-6 min-w-0 max-w-full cursor-pointer items-center gap-1.5 rounded-md px-2 text-[11px] text-muted transition-colors hover:bg-surface-overlay/60 hover:text-foreground disabled:cursor-default disabled:opacity-40"
          disabled={!session}
          aria-haspopup="menu"
        ...
        >
          ...
          <ChevronDown
            className={`shrink-0 text-muted/60 transition-transform ${menuOpen ? "rotate-180" : ""}`}
            size={12}
          />
        </button>
        {menuOpen && (
