        ...
        <div className="flex flex-col gap-4 p-5">
          <div className="flex items-center gap-3">
            <span
              className={`flex size-10 shrink-0 items-center justify-center rounded-full ${
                highRisk ? "bg-warning/15 text-warning" : "bg-accent/15 text-accent"
              }`}
            >
              {highRisk ? <CircleAlert size={20} /> : <Check size={20} />}
            </span>
            <div className="min-w-0 flex-1">
              <h2
                id={titleId}
          ...
          <div className="flex gap-2">
            <button
              ...
              autoFocus={highRisk}
              onClick={() => void respond("cancelled")}
              className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-lg border border-border bg-surface/50 text-xs font-medium text-foreground/80 transition-colors hover:bg-surface-overlay disabled:cursor-not-allowed disabled:opacity-45"
            >
              <X size={14} />
              {t("commonCancel")}
            </button>
