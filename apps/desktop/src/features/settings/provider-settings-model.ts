...
export function automaticThinkingConfig(
  ...
  // detected 没返回 thinkingLevelMap 时(模型 ID 不匹配已知档案),用默认全映射:
  // 每个等级映射到自身,让 max/xhigh 等都可用且原样传给 provider(anthropic 支持 effort max)。
  const defaultMap = Object.fromEntries(
    THINKING_LEVELS.map((level) => [level, level]),
  ) as DiscoveredProviderModel["thinkingLevelMap"];
  return {
    reasoning: true,
    thinkingLevelMap: detected.thinkingLevelMap ?? defaultMap,
...
export function providerSaveFailureMessage(message: string, provider: ProviderDraft): string {
  if (
    ...
    return "LXCode Host must be restarted before saving Models URL or compatibility overrides. Restart Host in Host settings, then save again.";
  }
  return message;
}
