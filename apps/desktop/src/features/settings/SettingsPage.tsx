...
import { UseCasesSettings } from "./UseCasesSettings";
import { BuiltinExtensionsSettings } from "./BuiltinExtensionsSettings";
import { WebSearchSettings } from "./WebSearchSettings";
import { AutomationTestSettings } from "./AutomationTestSettings";
          ...
          <div className="flex flex-col gap-4 rounded-lg border border-border p-4">
            <label className="flex items-center justify-between gap-4">
              ...
              <Select
                ...
                onChange={(v) => void patchDesktop({ theme: v as "light" | "dark" | "system" })}
                options={[
            ...
            <label className="flex items-center justify-between gap-4">
              ...
              <Select
                ...
                onChange={(v) => void patchDesktop({ language: v as "system" | "en" | "zh" })}
                options={[
                  { value: "system", label: t("commonSystem") },
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex min-w-0 items-center gap-1.5">
                <Select
                  ...
                  onChange={(v) => void patchDesktop({ terminalProfile: v as TerminalProfileId })}
                  options={[
                    { value: "auto", label: `${t("generalShellAutomatic")}${shellCatalog ? ` (${shellCatalog.automaticProfile.label})` : ""}` },
                    ...(shellCatalog?.profiles.map((p) => ({ value: p.id, label: p.label })) ?? []),
                    ...(desktopSettings?.terminalProfile &&
                    desktopSettings.terminalProfile !== "auto" &&
                    !shellCatalog?.profiles.some((p) => p.id === desktopSettings.terminalProfile)
                      ? [{ value: desktopSettings.terminalProfile, label: t("generalShellUnavailable", { id: desktopSettings.terminalProfile }), disabled: true }]
                      : []),
                  ]}
...
const SETTINGS_NAV: Array<{
  ...
  { id: "usecases", label: "navUseCases", icon: Layers },
  { id: "webSearch", label: "navWebSearch", icon: Globe },
  { id: "automationTest", label: "navAutomationTest", icon: FlaskConical },
  { id: "builtinExtensions", label: "navBuiltinExtensions", icon: Boxes },
...
/** 设置页 section 切换过渡:挂载时 fade + 轻微上滑。key=section 保证每次切换重挂。 */
function SettingsSectionTransition({ children }: { children: ReactNode }) {
  const ref = useRef<HTMLDivElement>(null);
  useGSAP(() => {
    if (!ref.current) return;
    gsap.fromTo(ref.current, { opacity: 0, y: 6 }, { opacity: 1, y: 0, duration: 0.22, ease: "power2.out" });
  }, []);
  return <div ref={ref} className="flex min-h-0 min-w-0 flex-1 flex-col">{children}</div>;
}
...
export function SettingsPage({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-surface" data-settings-root>
      <header
      ...
      <div className="flex min-h-0 flex-1">
        <nav className="w-52 shrink-0 border-r border-border bg-sidebar px-3 py-4">
          <p className="mb-2 px-2 text-[11px] font-medium text-muted">LXCode</p>
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          <SettingsSectionTransition key={section}>
            {section === "general" ? (
              <GeneralSettings />
            ) : section === "shortcuts" ? (
              <ShortcutsSettings />
              ...
              <WebSearchSettings />
            ) : section === "automationTest" ? (
              <AutomationTestSettings />
            ) : section === "builtinExtensions" ? (
              <BuiltinExtensionsSettings />
            ) : section === "packages" ? (
              <PackagesPage />
            ) : section === "host" ? (
              <HostSettings />
