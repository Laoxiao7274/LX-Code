...
export function summarizeHostFailure(
  ...
  const exitMatch = message.match(/^([\w][\w ]*Host) exited(?: \(([^)]+)\))?/);
  const exitDetail = exitMatch?.[2];
  const prefix = exitMatch ? `${exitMatch[1]} exited${exitDetail ? ` (${exitDetail})` : ""}` : "LXCode Host failed";
