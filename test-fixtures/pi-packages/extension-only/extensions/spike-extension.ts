 ...
 *
 * Env:
 *   LXCODE_SPIKE_NONCE  — required random nonce for this run
 *   LXCODE_SPIKE_MARKER — absolute path for the marker file
 */
...
export default function spikeExtension(pi: ExtensionAPI) {
  pi.on("session_start", async () => {
    const nonce = process.env.LXCODE_SPIKE_NONCE;
    const marker = process.env.LXCODE_SPIKE_MARKER;
    if (!nonce || !marker) {
      throw new Error("spike-extension: missing LXCODE_SPIKE_NONCE or LXCODE_SPIKE_MARKER");
    }
