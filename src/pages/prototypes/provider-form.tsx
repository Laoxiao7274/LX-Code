...
import type { Provider, Model, CustomHeader } from "./model-store";
...
/**
 * 提供商新增/编辑表单。
 * 支持 OpenAI 兼容 / Anthropic / 自定义 三种格式。
 */
export function ProviderForm({ provider, isAdding }: ProviderFormProps) {
  ...
  const addModel = () =>
    update({ models: [...form.models, { id: "", name: "", enabled: true }] });
  const updateModel = (i: number, patch: Partial<Model>) =>
    update({ models: form.models.map((m, idx) => (idx === i ? { ...m, ...patch } : m)) });
  const removeModel = (i: number) =>
    update({ models: form.models.filter((_, idx) => idx !== i) });

  const addHeader = () =>
    update({ headers: [...form.headers, { key: "", value: "" }] });
  const updateHeader = (i: number, patch: Partial<CustomHeader>) =>
    update({ headers: form.headers.map((h, idx) => (idx === i ? { ...h, ...patch } : h)) });
  ...
  const handleFetch = () => {
    setFetching(true);
    setTimeout(() => {
      fetchModels(form.id || "custom");
      setFetching(false);
    }, 800);
  };

  const handleSave = () => {
    const finalKind = kind === "custom" ? "custom" : "preset";
    saveForm({ ...form, kind: isAdding ? finalKind : form.kind, connected: !!form.apiKey });
  };
    ...
    <div className="surface absolute inset-2 z-50 flex flex-col overflow-hidden rounded-lg">
      ...
      <div className="flex h-12 items-center justify-between border-b border-border/60 px-4">
        <h2 className="text-[15px] font-semibold tracking-tight">
          {isAdding ? "新增提供商" : `编辑 ${form.name}`}
        </h2>
        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground" onClick={closeForm}>
          <X className="h-4 w-4" />
        </Button>
      </div>
                ...
                <div className="space-y-2">
                  {form.headers.map((h, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      />
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => removeHeader(i)}>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <div className="flex gap-1">
                ...
                <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs" onClick={addModel}>
              ...
              <div className="space-y-1.5">
                {form.models.map((m, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-lg border border-border/60 px-2.5 py-1.5">
                    ...
                    />
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => removeModel(i)}>
      ...
      <div className="flex h-12 items-center justify-end gap-2 border-t border-border/60 px-4">
        ...
        <Button onClick={handleSave} disabled={!form.id || !form.name}>
