        ...
        return (
          <div key={p.id} className="overflow-hidden rounded-lg border border-border/60">
            ...
            <div className="flex items-center gap-2.5 px-3 py-2.5">
              ...
              {/* 操作按钮 */}
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => openEdit(p.id)} title="编辑">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          m.enabled ? "bg-background/60" : "opacity-50",
                        )}
                      >
                        <button
                          type="button"
                          onClick={() => toggleModel(p.id, m.id)}
                          className={cn(
                            "flex h-4 w-4 shrink-0 items-center justify-center rounded-full border transition-colors",
                            isDefault ? "border-accent bg-accent" : "border-muted-foreground/30 hover:border-foreground/50",
                          )}
                        ...
                        {!isDefault ? (
                          <Button variant="ghost" size="sm" className="h-6 text-[11px] text-muted-foreground" onClick={() => setDefault(key)}>
                            设为默认
                          </Button>
                        ) : null}
                      </div>
