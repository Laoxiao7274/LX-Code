 ...
 *  2. Confirm:调 confirm_plan 工具弹窗等用户确认(默认是,回车开始)
 *  3. Execute:确认后才执行
 *  4. Verify:执行完调 run_verify 工具跑自动化测试,过才算完成
 *
 * 注册两个工具:
 *  - confirm_plan:ctx.ui.confirm 弹窗,返回用户是否同意
...
function buildWorkflowHint(cwd: string, pkg: PackageJson | null): string {
  ...
  const lines = [
    ...
    "",
    "### 2. Confirm(等用户确认)",
    "- 方案讲完后,调用 confirm_plan 工具(把方案摘要放进 message 参数)弹窗等用户确认。",
    "- 用户确认(默认是,回车)后才继续;用户拒绝就停下不要执行。",
...
export default function createTestFlowExtension(pi: ExtensionAPI): void {
  ...
  // 方案确认工具:弹窗等用户确认(方案已在对话里给出,弹窗只做确认,不重复方案)
  pi.registerTool({
    name: "confirm_plan",
    label: "方案确认",
    description:
      `改代码前向用户确认方案。方案你已经在上文讲清楚了,这里只需传一句简短确认语(如"是否开始执行?"),弹窗会显示这句让用户回车确认。不要把整个方案塞进 message。用户同意返回 true,拒绝返回 false。涉及改代码/跑命令的任务必须先调此工具确认。`,
    ...
    promptGuidelines: [
      ...
      "User defaults to yes (Enter); proceed only when it returns true",
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        const ok = await ctx.ui.confirm("方案确认", message);
        return {
          content: [{ type: "text", text: ok ? "用户已确认,开始执行。" : "用户已拒绝,停止执行。" }],
