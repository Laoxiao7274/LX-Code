...
export function generateCuratorPage(
	...
	const inlineData = safeInlineJSON({ queries, sessionToken, timeout, defaultProvider, searchProvider, summaryModels, defaultSummaryModel, availableProviders, autoApprove });
...
const SCRIPT = `(function() {
  ...
  var autoApprove = DATA.autoApprove === true;
  ...
  function requestSummary(indices, feedback) {
    ...
    postJson("/summarize", summarizePayload)
      ...
      .then(function(data) {
        ...
        updateStageUI();
        // autoApprove:摘要生成后自动提交,不等用户点 Approve
        if (autoApprove && getSummaryDraftText().length > 0) {
          setTimeout(function() { doApprove(); }, 300);
        }
      })
      .catch(function(err) {
        if (requestId !== summaryRequestSeq) return;
