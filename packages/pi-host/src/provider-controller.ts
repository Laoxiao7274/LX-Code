...
import { probeModelCapabilities, contextWindowFromCatalogItem } from "./extensions/probe-capabilities.js";
...
async function discoverModels(
  ...
  for (const item of items) {
    ...
    const visionDetected = detectModelVision(id, item);
    ...
    discovered.set(id, {
      ...
      reasoning,
      ...(thinkingLevelMap ? { thinkingLevelMap } : {}),
      input: existing?.input ?? visionDetected.input,
    ...
    "provider.probeModel": async (ctx) => {
      ...
      try {
        ...
        const captured = await withStableGraphRead({
          ...
          identity: server.identity,
          serviceGraphLock: server.serviceGraphLock,
          run: async (): Promise<ProviderFetchCapture | { error: HostError }> => {
            const config = await readModelsConfig(modelsPath);
            const raw = config.providers[providerId];
            if (!isObject(raw)) {
              return { error: createHostError("MODEL_NOT_FOUND", `Provider not found: ${providerId}`) };
        ...
        const { provider, apiKey } = captured.result.snapshot;
        // 真实探测思考/视觉能力(发最小测试请求)
        ...
        return {
          error: createHostError(
            "INTERNAL_ERROR",
            error instanceof Error ? error.message : "Could not probe model capabilities",
            { retryable: true },
          ),
        };
      }
