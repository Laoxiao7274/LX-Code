  ...
  return {
    ...
    command: {
      ...
      sourceInfo: {
        ...
        source: "npm:@lxcode/commands@2.0.0",
        scope: "user",
        origin: "package",
...
describe("extension invocation context", () => {
    ...
    const firstRun = withExtensionCommandOrigin(
      ...
      async () => {
        ...
        expect(origin?.origin).toMatchObject({
          ...
          extensionDisplayName: "@lxcode/commands",
          sourceKind: "package",
  ...
  it("generates stable opaque identities without exposing source paths", () => {
    ...
    const upgraded = normalizeExtensionIdentity(
      packageSourceInfo("/packages/commands/extensions/index.ts", "npm:@lxcode/commands@3.0.0"),
    );
    ...
    expect(current.extensionDisplayName).toBe("@lxcode/commands");
