...
function isModelSummary(value: unknown): boolean {
  return (
    isPlainObject(value) &&
    hasExactKeys(value, ["provider", "modelId", "name"], ["thinkingLevels", "reasoning"]) &&
    isString(value.provider) &&
    isString(value.modelId) &&
    isString(value.name) &&
    (value.thinkingLevels === undefined || isStringArray(value.thinkingLevels)) &&
    (value.reasoning === undefined || isBoolean(value.reasoning))
  );
}
...
function isBuiltinExtensionInfo(value: unknown): boolean {
  return (
    isPlainObject(value) &&
    hasExactKeys(value, ["id", "name", "enabled"]) &&
    isString(value.id) &&
...
export function validateMethodResultShape(method: HostMethod, result: unknown): string | null {
  ...
  switch (method) {
    ...
    case "workspace.initialize":
      return isPlainObject(result) &&
        hasExactKeys(result, ["git", "codegraph", "message"]) &&
        typeof result.git === "boolean" &&
        typeof result.codegraph === "boolean" &&
        typeof result.message === "string"
        ? null
        : "invalid workspace.initialize result";
    ...
    case "provider.setBuiltinModels":
      return isPlainObject(result) &&
        ...
        ? null
        : `invalid ${method} result`;
    case "builtinExtensions.list":
      return isPlainObject(result) &&
        hasExactKeys(result, ["extensions"]) &&
        Array.isArray(result.extensions) &&
        result.extensions.every(isBuiltinExtensionInfo)
        ? null
        : `invalid ${method} result`;
    case "builtinExtensions.setEnabled":
      return isPlainObject(result) &&
        hasExactKeys(result, ["extensionId", "enabled"]) &&
        isString(result.extensionId) &&
    ...
    case "provider.probeModel":
      return isPlainObject(result) &&
        hasExactKeys(result, ["providerId", "modelId", "reasoning", "vision", "embeddings", "contextWindow"], ["error"]) &&
        isString(result.providerId) &&
        isString(result.modelId) &&
        typeof result.reasoning === "boolean" &&
        typeof result.vision === "boolean" &&
        typeof result.embeddings === "boolean" &&
        (result.contextWindow === null || typeof result.contextWindow === "number")
        ? null
        : "invalid provider.probeModel result";
