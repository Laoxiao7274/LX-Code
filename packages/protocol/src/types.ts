...
export type ModelProbeResult = {
  ...
  /** 是否支持视觉(图片输入)。 */
  vision: boolean;
  /** 是否支持嵌入向量化(embeddings)。 */
  embeddings: boolean;
  /** 上下文窗口(来自 catalog max_model_len 等,探测不到则 null)。 */
...
export type DesktopSettings = {
  ...
  /** Per-use-case model overrides: useCase id -> "providerId/modelId", or "" to follow the default model. */
