...
function stageHostWithDeploy() {
  ...
  try {
    ...
    const deploy = timedStage("pnpm deploy production Host", () =>
      spawnSync("pnpm", ["--filter", "@lxcode/pi-host", "deploy", "--prod", deployedFrom], {
        cwd: root,
        encoding: "utf8",
        shell: true,
        env: process.env,
...
const releasePkg = {
  ...
  dependencies: deriveReleaseProductionDependencies(sdkEvidence, {
    "@lxcode/protocol": protoMeta.version,
  }),
};
...
if (protocolName !== "@lxcode/protocol") die("protocol package identity broken");
