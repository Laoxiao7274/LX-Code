...
function syncHostResources(destination) {
  ...
  for (const protocolRoot of [
    ...
    join(destination, "node_modules", "@lxcode", "protocol"),
  ]) {
...
function prepareDevHostResources() {
  const protocolMtime = ensurePackageBuild({
    label: "protocol",
    packageName: "@lxcode/protocol",
    sourceDirectory: protocolSrc,
    entry: protocolEntry,
  });
  ensurePackageBuild({
    label: "Pi Host",
    packageName: "@lxcode/pi-host",
...
async function main() {
  ...
  if (!existsSync(desktopExe)) {
    throw new Error(
      `Debug executable not found: ${desktopExe}\nRun "pnpm --filter @lxcode/desktop run tauri:dev" once to compile it.`,
    );
  }
  ...
  if (await isDesktopViteReady()) {
    ...
    ownsVite = true;
    vite = spawnPnpm(["--filter", "@lxcode/desktop", "run", "dev"]);
